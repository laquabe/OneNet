...
dataset_name = 'aquaint_test_noprompt_sum13B_13B'
# descriptions = read_json(dataset_path + 'kg_src/recall_entity_aida.json')
dataset = read_json(dataset_path + 'result/{}_noprompt.jsonl'.format(dataset_name))
'''listwise'''
# output_f = open(dataset_path + 'result/{}_test_prompt0_text_13B.jsonl'.format(dataset_name), 'w')
'''judge'''
output_f = open(dataset_path + 'result/judge/{}_noprompt.jsonl'.format(dataset_name), 'w')
instruction_dict = read_prompt('/data/xkliu/EL_code/LLM4EL/prompt/prompt.jsonl')
output_domain = 'llm_judge'
...
def main(
    ...
    for src_dict in tqdm(dataset):
        ...
        content = prompt_formula_judge(src_dict)
        dialogs.append(
